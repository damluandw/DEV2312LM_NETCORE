...
namespace DanhGiaRenLuyen_V6.Common
{
    public class ExportToExcel
    {
        public static void Export(string _filename, string _nameSheetDataSet, DataSet _dataSet)
        {
                    ...
                    try
                    {
                        if (_dataSet != null)
                        {
                            for (int i = 0; i < _dataSet.Tables.Count; i++)
                            {
                                ...
                                Excel.Worksheet _xlWorkSheet = (Excel.Worksheet)excel.Worksheets.get_Item(1);
                                Excel.Range formatRange;
                                formatRange = _xlWorkSheet.get_Range("A1", "AO500");
                                //format kiểu Text
                                formatRange.NumberFormat = "@";

                                setExcelVauleTable(_xlWorkSheet, _dataSet.Tables[i],  _nameSheetDataSet.Split(',')[i]);
                        ...
                        workbook.SaveAs(_filename, Type.Missing, Type.Missing, Type.Missing,
        ...
        private static void setExcelVauleTable(Excel.Worksheet _xlWorkSheet, DataTable _dateTable, string _nameSheet)
        {
            if (_nameSheet != "" && _nameSheet != null)
                _xlWorkSheet.Name = _nameSheet;
            int col = 0;
           if (_dateTable.Rows.Count > 0)
                {
                    _xlWorkSheet.Cells[1, 1] = "STT";
                    //Header
                    for (var x = 0; x < _dateTable.Rows[0].ItemArray.Count(); x++)
                    {
                        _xlWorkSheet.Cells[1, x + 2] = _dateTable.Columns[x].ColumnName;

                    }
                    for (var r = 0; r < _dateTable.Rows.Count; r++)
                    {
                        //Cột STT
                        _xlWorkSheet.Cells[r + 2, 1] = r + 1;
                        //Dữ liệu
                        for (var x = 0; x < _dateTable.Rows[r].ItemArray.Count(); x++)
                        {
                            //Format dữ liệu xuất excel
                            string tmp = _dateTable.Rows[r].ItemArray[x].ToString();
                            //formatCell(_xlWorkSheet, tmp, r + 2, x + 2);
                            _xlWorkSheet.Cells[r + 2, x + 2] = _dateTable.Rows[r].ItemArray[x];
                        }
                    }
                }
        ...
        public static void Export(string _filename, string _fieldsDataSet, string _nameSheetDataSet, DataSet _dataSet)
        {
            Excel.Application excel = new Excel.Application();
            var workbook = (Excel._Workbook)(excel.Workbooks.Add(Missing.Value));
            try
            {
                if (_dataSet != null)
                {
                    ExportDataSet(excel, workbook, _dataSet, _fieldsDataSet, _nameSheetDataSet);
                }
                workbook.SaveAs(_filename, Type.Missing, Type.Missing, Type.Missing,
                    Type.Missing, Type.Missing, Excel.XlSaveAsAccessMode.xlNoChange,
                    Type.Missing, Type.Missing, Type.Missing, Type.Missing,
        ...
        public static void Export(string _filename, string _fieldsDatatable, string _nameSheetDataTable, DataTable _dataTable)
        {
            Excel.Application excel = new Excel.Application();
            var workbook = (Excel._Workbook)(excel.Workbooks.Add(Missing.Value));
            try
            {
                if (_dataTable != null)
                {
                    ExportDataTable(excel, workbook, _dataTable, _fieldsDatatable, _nameSheetDataTable);
                }
        ...
        private static void ExportDataSet(Excel.Application excel, Excel._Workbook _workbook, DataSet _dataSet, string _fields, string _nameSheet)
        {

            for (int i = 0; i < _dataSet.Tables.Count; i++)
            {
                //Tạo sheet mới
                if (_workbook.Sheets.Count <= i)
                {
                    _workbook.Sheets.Add(Type.Missing, Type.Missing, Type.Missing,
                                        Type.Missing);
                }
                ...
                //format kiểu Text
                formatRange.NumberFormat = "@";
                setExcelVauleTable(_xlWorkSheet, _dataSet.Tables[i], _fields, _nameSheet.Split(',')[i]);



            }
        }
        private static void ExportDataTable(Excel.Application excel, Excel._Workbook _workbook, DataTable _dateTable, string _fields, string _nameSheet)
        {
            ...
            //format kiểu Text
            formatRange.NumberFormat = "@";
            setExcelVauleTable(_xlWorkSheet, _dateTable, _fields, _nameSheet);
        }

        private static void setExcelVauleTable(Excel.Worksheet _xlWorkSheet, DataTable _dateTable, string _fields, string _nameSheet)
        {
            if (_nameSheet != "" && _nameSheet != null)
                _xlWorkSheet.Name = _nameSheet;
            int col = 0;
            if (_fields != "")
            {
                ...
                {
                    _xlWorkSheet.Cells[1, 1] = "STT";
                    for (var i = 0; i < _dateTable.Rows[0].ItemArray.Count(); i++)
                    {
                        foreach (string field in _fields.Split(','))
                        {
                            if (_dateTable.Columns[i].ColumnName == field)
                            {
                                col += 1;
                                _xlWorkSheet.Cells[1, col + 1] = field;
                            }
                        }
                    }

                    for (int r = 0; r < _dateTable.Rows.Count; r++)
                    {
                        int col2 = 1;
                        for (var c = 0; c < _dateTable.Rows[r].ItemArray.Count(); c++)
                        {

                            _xlWorkSheet.Cells[r + 2, 1] = r + 1;
                            foreach (string field in _fields.Split(','))
                            {
                                if (_dateTable.Columns[c].ColumnName == field)
                                {
                                    col2 += 1;
                                    //Format dữ liệu xuất excel
                                    string tmp = _dateTable.Rows[r].ItemArray[c].ToString();
                                    //formatCell(_xlWorkSheet, tmp, r + 2, col2);
                                    _xlWorkSheet.Cells[r + 2, col2] = _dateTable.Rows[r].ItemArray[c];
                                }
                            }
                        }
                    }
        ...
        // Lọc số kiểu float
        private static bool IsNumberFloat(string pText)
        {
        ...
        private void ExportExcel(DataTable dgResults)
        {
            ...
            {
                iCol++;
                xlWorkSheet.Cells[2, iCol] = dgResults.Columns[iCol - 1].ColumnName;
            }
        ...
        ((Microsoft.Office.Interop.Excel.Range)xlWorkSheet.Columns["A", Type.Missing]).ColumnWidth = 7.00;
            ...
            string fileName = "~/YourFileNameHere.xls";
            ...
            String FilePath = "~/YourFileNameHere.xls";
